...
| **E22** | Decode typed `[]byte` from raw JSON string bytes and call `base64.StdEncoding.Decode` directly, avoiding the escaped-string materialization before base64 decode. | Escaped byte-slice bench (`"\\u0053GV..."`, 3s × 3): ~150–160 ns/op, 64 B/op, 3 allocs/op → **123–135 ns/op, 40 B/op, 2 allocs/op**. Unescaped path stays at 2 allocs. Red allocation guard and full tests pass. | ✓ |
| **E23** | Symmetric encode cleanup for typed/top-level `[]byte`: add a direct `[]byte` fast path and grow the base64 output buffer without appending a temporary zero slice. | `Marshal([]byte("Hello, world!"))` bench (3s × 3): **72–77 ns/op, 48 B/op, 2 allocs/op** vs stdlib 100–104 ns/op. Allocation guard catches the old 3-alloc temp-slice path. | ✓ |
| **E24** | Restore `encoding/json` default escaping contract: strings and struct field names must HTML-escape `<`, `>`, `&`, U+2028/U+2029; invalid UTF-8 must become `\ufffd`; `MarshalJSON` output must be compacted and HTML-escaped before append. | Red parity tests caught raw HTML chars, dropped invalid-UTF8 replacement, and un-compacted `MarshalJSON` bytes. Broadened SIMD/SWAR string scan to break on HTML/high-bit bytes and changed slow path to chunk-copy safe spans. `go test ./...`, amd64/arm64 test-binary compile pass. Cost: scanner microbench drops to ~10.8 GB/s from the old quote/control-only ~23.6 GB/s; encode twitter now ~0.92 ms vs sonic ~0.82 ms in a noisy 1s × 2 run, while small/citm/canada remain ahead. | ✓ correctness |
| **E25** | Recover E24 twitter encode cost by specializing for the actual corpus shape: 95% of twitter strings are shorter than 64 B and 95% are clean ASCII. | Added a `stringSafeSet` short-string/tail scanner (n ≤ 32) and byte-level UTF-8 validation in the slow path. Profile: `scanStringSWAR` drops from ~20-22% CPU to ~2.6%; `DecodeRuneInString` disappears. Twitter encode recovers to **0.83-0.88 ms** in 2s × 3 samples, comparable with sonic's noisy **0.84-0.96 ms** range; citm and canada remain ahead. Focused escaping tests and `go test ./...` pass. | ✓ |
